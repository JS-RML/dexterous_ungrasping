...
gbs.init("gen_release_palm.yaml") #load config as global variables
...
if __name__ == '__main__':
    try:
        ...
        table_height_wrt_world = 0.011-0.02 # black carton on aluminium 0.04-0.02 with black carton:0.114-0.02 #box on table:0.111-0.02  #table: -0.02

        print "init pose"
        #pose = [-0.3, 0.630, table_height_wrt_world+tcp2fingertip+object_length-delta_0, 0.7071, 0, -0.7071, 0]
        pose = [0.603, -0.812, table_height_wrt_world+tcp2fingertip+object_length-delta_0, 0.7071, 0, -0.7071, 0]
        #0.603, -0.797
        ...
        Robotiq.goto(robotiq_client, pos=0.032, speed=gbs.config['gripper_speed'], force=gbs.config['gripper_force'], block=False)
        ...
        print "move down"
        motion_primitives.set_pose_relative([0, 0, -0.048])
        raw_input()
        ...
        width = regrasp.generalized_release(np.multiply(axis, -1),20,0.028,0.02) #20,0.025
        raw_input()

        center = [P_w_center[0], P_w_center[1]+0.0, P_w_center[2]-0.048+0.003] 

        dynamixel.set_length(125)
        Robotiq.goto(robotiq_client, pos=width*1.0, speed=gbs.config['gripper_speed'], force=gbs.config['gripper_force'], block=False)
