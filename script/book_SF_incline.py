...
gbs.init("book_SF.yaml") #load config as global variables
...
if __name__ == '__main__':
    try:
        ...
        raw_input()
        motion_primitives.set_pose_relative([0, -0.7, 0]) #0.25
        ...
        print "move down"
        motion_primitives.set_pose_relative([0, 0, -0.038])
        raw_input()

        print "regrasp"
        width = regrasp.regrasp(np.multiply(axis, -1), int(10), 0.05)
        #raw_input()
        ...
        center = [P_w_center[0], P_w_center[1]+0.0, P_w_center[2]-0.038] 
