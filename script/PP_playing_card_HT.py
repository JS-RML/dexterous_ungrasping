...
if __name__ == '__main__':
    ...
    try:
        ...
        pre_pick_pose = [0.464, 0.0, 0.416, 0.7071, -0.0, -0.7071, 0.0]
        pick_pose = [0.464, 0.0, 0.341, 0.7071, -0.0, -0.7071, 0.0]
        
        for i in range(1):
            for j in range(1):
                regrasp_pose=[0.45, -0.317, 0.342+0.013*(i/13)-0.007, -0.7071, -0.0, 0.7071, 0.0]
                ...
                rot_tool0 = [p.orientation.x, p.orientation.y, p.orientation.z, p.orientation.w] 
                T_wg = tf.TransformerROS().fromTranslationRotation(trans_tool0, rot_tool0)
                P_g_center = [tcp2fingertip, -0.035/2, 0, 1]
                P_w_center = np.matmul(T_wg, P_g_center)
                center = [P_w_center[0], P_w_center[1], P_w_center[2]]
...
# Object dimension
object_thickness: 0.000 #0.01 # object thickness in meters
object_length: 0.087 # object length in meters

# Initial configuration
delta_0: 0.052 #0.0425 # distance from fingertip to object tip within gripper
theta_0: 70.0
