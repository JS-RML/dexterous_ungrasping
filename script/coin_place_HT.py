...
gbs.init("coin_place_HT.yaml") #load config as global variables
...
if __name__ == '__main__':
    try:
        ...
        pose = [0.59, -0.8, table_height_wrt_world+tcp2fingertip+object_length-delta_0, 0.7071, 0, -0.7071, 0]
        #0.603, -0.797
        ...
        raw_input()
        motion_primitives.set_pose_relative([0, 0.193, 0]) #0.25
        ...
        print "move down"
        motion_primitives.set_pose_relative([0, 0, -0.0452])

        raw_input()

        print "regrasp"
        width = regrasp.regrasp(np.multiply(axis, -1), int(psi_regrasp), 0.02)
        #raw_input()

        Robotiq.goto(robotiq_client, pos=width*1.16, speed=0.01, force=gbs.config['gripper_force'], block=False)
        ...
        center = [P_w_center[0], P_w_center[1]+0.0, P_w_center[2]-0.0452] 

        raw_input()
        print "tilt"
        tilt.tilt(center, axis, int(theta_tilt), 0.03)
