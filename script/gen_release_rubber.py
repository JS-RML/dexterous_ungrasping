...
gbs.init("gen_release_rubber.yaml") #load config as global variables
...
if __name__ == '__main__':
    try:
        ...
        table_height_wrt_world = 0.013-0.02 # black carton on aluminium 0.04-0.02 with black carton:0.114-0.02 #box on table:0.111-0.02  #table: -0.02

        print "init pose"
        #pose = [-0.3, 0.630, table_height_wrt_world+tcp2fingertip+object_length-delta_0, 0.7071, 0, -0.7071, 0]
        pose = [0.35, -0.75, table_height_wrt_world+tcp2fingertip+object_length-delta_0, 0.7071, 0, -0.7071, 0]
        #0.603, -0.797
        ...
        raw_input()
        motion_primitives.set_pose_relative([0.2, 0, 0])
        ...
        print "move down"
        motion_primitives.set_pose_relative([0, 0, -0.044])
        raw_input()

        motion_primitives.set_pose_relative([0, -0.05, 0])
        raw_input()
        motion_primitives.set_pose_relative([0, 0, -0.003])
        raw_input()
        ...
        regrasp.generalized_release(np.multiply(axis, -1),23,0.041,0.02)
        raw_input()

        center = [P_w_center[0], P_w_center[1]-0.05, P_w_center[2]-0.047] 
