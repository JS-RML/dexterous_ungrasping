...
if __name__ == '__main__':
    ...
    try:
        ...
        sim = config['sim']
        table_height_wrt_world = -0.02 #0.131-0.02 #battery box on table
        ...
        print "init pose"
        pose = [0.507, -0.7, table_height_wrt_world+tcp2fingertip+object_length-delta_0, 0.7071, 0, -0.7071, 0]
        ...
        print "tilt"
        tilt.tilt([P_w_center[0], P_w_center[1], P_w_center[2]], axis, 45, tcp_speed)

        raw_input()

        motion_primitives.set_pose_relative([0, -0.157, 0])
        ...
        regrasp.regrasp(axis, 32, tcp_speed)
        rospy.sleep(0.5)

        #raw_input()

        center = [P_w_center[0], P_w_center[1]-0.157, P_w_center[2]-0.01]
        print "tilt"
        # Tilt
        tilt.tilt(center, np.multiply(axis, -1), 10, tcp_speed)
        rospy.sleep(0.5)
        
        print "slide"
        # Surface Slide
        motion_primitives.set_pose_relative([0, 0, -0.006])
        rospy.sleep(0.5)
        
        print "tuck"
        # Push-Tuck        
        # tuck.push_tuck(np.multiply(axis, -1), int(tuck_angle), 0.01, tcp_speed, 143)
        tuck.rotate_tuck(axis, 5, 0.01, tcp_speed)
        rospy.sleep(0.5)
        
        motion_primitives.set_pose_relative([0, 0, 0.01])
        motion_primitives.set_pose_relative([0, 0.1, 0])
